    ...
    <div class="d-flex justify-content-center">
        <div class="w-50 my-3 p-1 rounded shadow-lg">
                            ...
                            <div class="modal-dialog">
                                <div class="modal-content">
                                    <div class="modal-header">
                                        <h1 class="modal-title fs-5" id="exampleModalLabel">Edit Foto</h1> <button
                                            type="button" class="btn-close" data-bs-dismiss="modal"
                                            aria-label="Close"></button>
                                    </div>
                                    <form action="{{ route('photo.update', $data->id) }}" method="POST"> @csrf
                                        @method('put') <div class="modal-body">
                                            <div class="form-roup mb-3">
                                                <label for="judul_foto" class="form-label">Judul Foto</label> <input
                                                    type="text" name="judul_foto" id="judul_foto" class="form-control"
                                                    value="{{ $data->judul_foto }}">
                                            </div>
                                            <div class="form-group mb-3"> <label for="deskripsi_foto"
                                                    class="form-label">Deskripsi Foto</label>
            ...
            <img class="img-fluid mx-auto d-block" src="{{ asset('storage/' . $data->lokasi_file) }}"
                alt="{{ $data->judul_foto }}" width="300" height="300">
            <div class="d-flex justify-content-between align-items-center">
                ...
                {{-- Delete Photo --}} <form action="{{ route('photo.delete', $data->id) }}" method="POST">
                    @csrf
                    @method('delete')
                        ...
                        <div class="d-flex justify-content-between align-items-center">
                            ...
                            <div class="d-flex align-items-center">
                                @if (Auth::user()->id == $comment->user->id)
                                            ...
                                            <div class="modal-content">
                                                ...
                                                <form action="{{ route('comment.update', $comment->id) }}"
                                                    ...
                                                    @csrf @method('put') <div class="modal-body">
                                                        <div class="form-group mb-3"> <label for="isi_komentar"
                                                                class="form-label">Isi Komentar</label> <input
                                                                type="text" name="isi_komentar" id="isi_komentar"
                                                                class="form-control"
                                                                value="{{ $comment->isi_komentar }}"> </div>
                                                    </div>
                                    ...
                                    <form action="{{ route('comment.delete', $comment->id) }}" method="POST"> @csrf
                                        @method('delete') <button class="btn"
